...
while (x ~= 1) && (x ~= 2)
    ...
    if x == 1
        load ExcitationM
    elseif x == 2
        Ts_3ord = 0.01; % Sampling time for u3211
...
% Input Assigned
t_3ord=0:simulation_time/length(simulation_data.q):simulation_time;
...
theta0 = [Xu Xq Mu Mq Xd Md] + 0.1 * (-0.5 + rand(1,6)).*[Xu Xq Mu Mq Xd Md];
